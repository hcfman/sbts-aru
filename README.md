...
- **Any Raspberry Pi**, the raspberry pi zero series have the lowest power usage for portable usage. Raspberry Pi Zero W is typically less than 20 euros.
- A GPS capable of providing data read/write and PPS signals. These include, but are not limited to **ublox neo 6m, 7m and 8m**, as well as **Adafruit ultimate breakout boards**. The ublox modules can be obtained from aliexpress for around 7 euros. My choice would be the neo 7m.
- A **16-bit USB sound card** compliant with jackd. Typically all the **cirrus logic cm108** based ones work as well as the **General Plus** based ones. These can be as cheap as less than one euro. My preference is the General Plus based ones as they tend to be able to provide higher mic boost gain
